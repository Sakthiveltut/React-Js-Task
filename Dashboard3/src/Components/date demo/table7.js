...
function Table4() {
  ...
  const handleGroupByTaskChange = () => {
    setGroupByTask(!groupByTask);
    setSelectedTasks([]); // Clear selected tasks when switching group by task
  };

  const filteredData = groupByTask
    ? data.filter((row) => selectedTasks.includes(row.task))
    : data;
  ...
  return (
    <div className="card" style={{}}>
      <div className="card-body">
        <label>
          <input type="checkbox" checked={groupByTask} onChange={handleGroupByTaskChange} />
          Group by Task
        </label>
